...
class TiDEModel(nn.Module):
    ...
    def __init__(self, config: TiDEConfig):
        ...
        # The encoder expects all configured features
        enc_dim = L * output_size  # Target variable features

        # Add past features contribution if they exist
        if past_feature_size > 0:
            if config.past_feature_projection_size > 0:
                enc_dim += L * config.past_feature_projection_size
            else:
                enc_dim += L * past_feature_size

        # Add future features contribution if configured
        # These will be created as zeros if not provided
        if future_input_size > 0:
            if config.future_forcing_projection_size > 0:
                enc_dim += H * config.future_forcing_projection_size
            else:
                enc_dim += H * future_input_size

        # Add static features if configured
        if static_size > 0:
            enc_dim += static_size
    ...
    def forward(
        ...
        # Process future covariates - create zeros if configured but not provided
        if self.config.future_input_size > 0:
            if future is not None and future.shape[-1] > 0:
                ...
                # Create zero tensor for future features if not provided
                future = torch.zeros(B, H, self.config.future_input_size, device=x.device, dtype=x.dtype)
                future_proj = self.future_projection(future) if self.future_projection is not None else future
        else:
            future_proj = None

        # Process static features - create zeros if configured but not provided
        if self.config.static_size > 0 and static is None:
            static = torch.zeros(B, self.config.static_size, device=x.device, dtype=x.dtype)
